...
- Аудит `static_cast<uint16_t/uint8_t>` по `core/`: усечение MyINFO закрыто гардом `> UINT16_MAX`, ExtJSON ограничен `MAX_MYINFO_LEN*10 ≤ 5120`, разбиения OpForceMove/HubCommands читают в пределах буфера с валидацией длин, `GenerateMyInfo*` строят в 256 КБ буфер из полей ≤ 64 КБ — новых усечений не найдено.
